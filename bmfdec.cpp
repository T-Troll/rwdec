...
int main(int argc, char* argv[]) {
  ...
  if (file != INVALID_HANDLE_VALUE) {
      ...
      while ((linePos = content.find("Device(", linePos)) != string::npos) {
          ...
          size_t nextDev = content.find("Device(", linePos + 7);
          string deviceInfo = content.substr(linePos, nextDev != string::npos ? nextDev - linePos : content.length() - linePos);
          size_t nextPos = 0;
          while ((nextPos = deviceInfo.find("0x46, 0x4F, 0x4D, 0x42", nextPos)) != string::npos) {
              size_t endBlockPos = deviceInfo.find("}", nextPos);
              line = deviceInfo.substr(nextPos, endBlockPos - nextPos);
              nextPos = endBlockPos;
              size_t inPos = 0;
              lin = 0;
              // parse block
  ...
  return 0;// process_data(pout, lout);
}
